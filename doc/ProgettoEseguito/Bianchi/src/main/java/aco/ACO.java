...
public class ACO
{
    ...
    public ACO(double alpha, double beta, double exploitation, int seed1, int seed2, int iterationNumber, int antsNumber)
    {
        this.randomGenerator = new Random(seed1);
        this.rand = new Random(seed2);
    ...
    public Tour antColony(Tour oldTour)
    {
        ...
        int iteration;
        for(iteration = 0; iteration < iterationNumber && (System.currentTimeMillis() - startTime) / 1000 < 175; iteration++)
        {
            ...
            {
                //for (int i = 0; i < antsNumber; i++) //add cities for all ants
                for (Ant a : ants)
                {
